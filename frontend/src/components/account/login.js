...
class Login extends Component{
    ...
    handleSubmit(event){
        AuthService.login(this.state.username, this.state.password, output =>{
    ...
    render(){
        let auth = false;
        AuthService.checkLogin(resp => {auth = resp;});
        if(this.state.loggedIn === true || auth){
            return <Redirect to='/' />
        }else{
            return(
                <div className='logindiv loginpage' style={{backgroundImage: `url(${loginpage})`}}>
                    <form className="signupBox login" onSubmit={this.handleSubmit} >
                        ...
                        <h2 className='networkmotto'>A place to share knowledge and better understand the world</h2>
                        <h3 style={{paddingLeft: '85px'}}>Login</h3>
                        <div className="formElem">
                            <input type="text" name="username" className="textfield" id="username" value={this.state.username} onChange={this.handleChange} required />
                            <label htmlFor="username">Username</label>
                        </div>
                        <div className="formElem">
                            <input type="password" name="password" className="textfield" id="password" value={this.state.password} onChange={this.handleChange} required />
